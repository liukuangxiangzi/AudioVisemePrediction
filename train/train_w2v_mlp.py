...
def define_model(trainx, resume_training, load_model_dir):
    if not resume_training:
        ...
        l1 = Dense(128,
                   ...
                   name='lm_Dense1', activation='relu')(net_in)
...
def train_model(model, trainx, trainy, testx, testy, epochs, batch_size, log_dir, save_model_dir):
    ...
    logging.info('Starting w2v MLP model training')
    ...
    plot_model(model, to_file='../model_w2v_MLP_plot.png', show_shapes=True, show_layer_names=True)
    ...
    # Save model to file
    model.save(os.path.join(save_model_dir, 'w2v_mlp_epoch{}_bs{}.h5'.format(epochs, batch_size)))
...
def main():
    ...
    parser.add_argument('--save-model-dir', type=str, default='model/MLP', help='directory where trained model will be saved')
