...
def train_model(model, trainx, trainy, testx, testy, epochs, batch_size, log_dir, save_model_dir):
    ...
    logging.info('Starting w2v LSTM model training')
    ...
    plot_model(model, to_file='../model_w2v_lstm_plot.png', show_shapes=True, show_layer_names=True)
    ...
    model.save(os.path.join(save_model_dir, 'w2v_lstm_epoch{}_bs{}.h5'.format(epochs, batch_size)))
...
def main():
    ...
    parser.add_argument('--save-model-dir', type=str, default='model/LSTM', help='directory where trained model will be saved')
